...
public:
	...
	T get(int t, int p) {
		...
		if(t) {
			...
			for(; p < sz(bit); p += p & -p) answer += bit[p];
		}
		return answer;
	}
};
...
void process() {
	...
	lint numInversion = 0;
	FenwickTree<lint> bit(n), ft(n);

    for(int i = 1; i < k; i++) {
    	numInversion += bit.get(0, a[i] + 1);
    	bit.update(0, a[i], 1);
    	ft.update(1, a[i], 1);
    }
    ...
    for(int i = k; i <= n; i++) {
    	...
    	bit.update(0, a[i - k + 1], -1);
    	ft.update(1, a[i - k + 1], -1);
    	numInversion -= ft.get(1, a[i- k + 1] - 1);
    }
}
