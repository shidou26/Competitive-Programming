...
template<class T> using OrderedTree = 
...
void process() {
	...
	FenwickTree<lint> bitCounter(sz(cps)), bitSum(sz(cps));
	OrderedTree<lint> ost;

	for(int i = 1; i < k; i++) {
		bitCounter.update(compress(a[i]), 1);
		bitSum.update(compress(a[i]), a[i]);
		ost.insert(a[i]);
	}
	...
	for(int i = k; i <= n; i++) {
		...
		lint median = *ost.find_by_order(k / 2);
		cout << bitCounter.get(compress(median)) * median - bitSum.get(compress(median)) + bitSum.getComp(compress(median)) - bitCounter.getComp(compress(median)) * median << " ";

		bitCounter.update(compress(a[i - k + 1]), -1);
		bitSum.update(compress(a[i - k + 1]), -a[i - k + 1]);
		ost.erase(ost.lower_bound(a[i - k + 1] - 1));
	} 
