...
lint prefixA[N], prefixB[N], prefix[N];

bool satisfy(int l, int r) {
	return prefix[r] - prefix[l - 1] - (prefixB[r] - prefixB[l - 1]) * prefixA[l - 1] <= c;
}
...
void process() {
	for(int i = 1; i <= n; i++) {
		...
		prefix[i] = prefix[i - 1] + (s[i] == 'b' ? prefixA[i] : 0);
	}

   	int maxLength = 0;
    for(int i = 1, j = 1; i <= n; i++) {
    	while(j <= i && !satisfy(j, i)) j++;
    	maximize(maxLength, i - j + 1);
    }
    cout << maxLength << endl;
}
