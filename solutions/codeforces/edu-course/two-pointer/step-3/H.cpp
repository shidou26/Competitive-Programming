...
void process() {
	sort(a + 1, a + 1 + n, greater<lint>()); 
	sort(b + 1, b + 1 + m, greater<lint>());
    int j = 0;
    lint maxCost = 0, currentWeight = 0, currentCost = 0;

    j = currentWeight = currentCost = 0;
    while(j < m && currentWeight + B <= S) currentWeight += B, currentCost += b[++j];
    for(int i = 1; i <= n; i++) {
    	currentWeight += A; currentCost += a[i];
    	while(j >= 1 && currentWeight > S) currentWeight -= B, currentCost -= b[j--];
    	if(currentWeight <= S) maximize(maxCost, currentCost); 
    }

    j = currentWeight = currentCost = 0;
    while(j < n && currentWeight + A <= S) currentWeight += A, currentCost += a[++j];
    for(int i = 1; i <= m; i++) {
        currentWeight += B; currentCost += b[i];
        while(j >= 1 && currentWeight > S) currentWeight -= A, currentCost -= a[j--];
        if(currentWeight <= S) maximize(maxCost, currentCost); 
    }
